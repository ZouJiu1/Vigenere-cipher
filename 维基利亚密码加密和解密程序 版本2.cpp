...
public:
    void choic(void)
    {
        ...
        for(i=0;i<l;i++)
        {
            if((int)enc[i]<=90&&(int)enc[i]>=65)
            {
                enc[i]=(char)((int)enc[i]+32);
            }
        }
        for(a=1;a<=6;a++)
        {
            for(i=0;i<l;i++)
            {
                if((int)enc[i]<65||(int)enc[i]>122||((int)enc[i]<97&&(int)enc[i]>90))
                {
                    for(j=i;j<l;j++)
                    {
                        enc[j]=enc[j+1];
...
{
    char h;
    cout<<"加密字符串请输入E，解密字符串请输入D。\n请选择功能：";
    cin>>h;
    if(h=='E')
    {
        fflush(stdin);
        Forth For;
        cout<<"输入需要编辑成一段，不要有换行符否则需要分段加密，其它内容不用变。"<<endl;
        cout<<"请输入（可复制粘贴）待加密的英文文段：";
